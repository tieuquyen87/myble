...
void char2_read_handler(esp_gatts_cb_event_t event, esp_gatt_if_t gatts_if, esp_ble_gatts_cb_param_t *param) {
	ESP_LOGI(GATTS_TAG, "char2_read_handler %d -%x \n", param->read.handle, param->read.handle);
...
//       esp_ble_gatts_send_response(gatts_if, param->write.conn_id, param->write.trans_id, ESP_GATT_OK, NULL);
    ...
    case ESP_GATTS_CREATE_EVT:
        ESP_LOGI(GATTS_TAG, "CREATE_SERVICE_EVT, status %d,  service_handle %d - %x \n", param->create.status, param->create.service_handle,param->create.service_handle);
...
//                                     ESP_GATT_PERM_READ | ESP_GATT_PERM_WRITE, NULL, NULL);
    ...
    case ESP_GATTS_CONNECT_EVT:
		...
		printf("\nCONNECTEDDDDDDDDDDDDDDDDDDDDDDDDDD\n");
		timeblink = 2000;
		printf("\t%s, timeblink = %d\n",__func__,timeblink);
        break;
    case ESP_GATTS_DISCONNECT_EVT:
        esp_ble_gap_start_advertising(&ble_adv_params);
		printf("\nDISCONNECTEDDDDDDDDDDDDDDDDDDDDDDDDDD\n");
		timeblink = 200;
		printf("\t%s, timeblink = %d\n",__func__,timeblink);
        break;
